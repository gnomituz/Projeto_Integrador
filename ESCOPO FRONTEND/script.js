...
function atualizarTarefas(){
    ...
    tarefasSalvas.forEach((tarefa, index) => { 
        ...
        novoCard.innerHTML = `
        <span>${tarefa}</span>
        <button onclick="editarTarefa(${index})">Editar Cadastro</button>
        <button onclick="deletarTarefa(${index})">Encerrar atendimento</button>
        `;
...
window.editarTarefa = function(index){
    let novaTarefa = prompt("Edite a sua tarefa: ", tarefasSalvas[index]);
    if(novaTarefa !== null) {
